...
Uso de IA: Ferramentas de IA Generativa (Gemini/ChatGPT) foram consultadas para suporte técnico na manipulação de tensores com a biblioteca NumPy, especificamente na transposição de eixos em matrizes 3D (np.swapaxes). A IA auxiliou no brainstorming para a criação de um cenário de teste simplificado e na estruturação inicial desta documentação, a qual foi expandida e redigida em conjunto por mim. A implementação lógica da classe MecanismoAtencao, a normalização via funcao_softmax e a validação através de testes unitários (assert) foram realizadas integralmente por mim. O trabalho reflete meu entendimento sobre a escalabilidade do produto escalar e a representação visual da atenção, garantindo que os pesos calculados coincidam com os valores teóricos esperados, conforme as regras do contrato pedagógico.